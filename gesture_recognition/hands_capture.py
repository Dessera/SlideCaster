...
class HandsCapture:
    ...
    """
        Mark the curent frame, if there is a gesture :)
    """
    def get_marked_frame(self, original_frame: cv2.Mat) -> cv2.Mat:
        frame = cv2.cvtColor(original_frame, cv2.COLOR_BGR2RGB)
        if self.shell_flip: # Flip the image, for some camera transfer an flipping image
            frame = cv2.flip(frame, 1)
        results = self.hands.process(frame)  # process()是手势识别最核心的方法，通过调用这个方法，将窗口对象作为参数，mediapipe就会将手势识别的信息存入到res对象中
        # Convert the image back, for inside the process it cvt back to BGR
        # which shell be incorrect for displaying videos
